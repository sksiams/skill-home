 jQuery(document).ready(function(){
	
	jQuery('.srch-icon').click(
	
		function(){
			jQuery('.menu-area ul').css({'margin-left':'25%'});
			jQuery('.srch-inpt').fadeIn(
				function(){
					
					jQuery('.header-top').mouseleave(
					
						function(){
							
							jQuery('.srch-inpt').hide();
							jQuery('.menu-area ul').css({'margin-left':'44%'});
			
							
						}
					
					);
					
					
				}
			
			);
	
		}

	);
	

	jQuery('.rspnsv-bar').click( 
	
		function(){
			
			jQuery('.rspnsv-menu').fadeToggle();
			
			
			
		}
	
	
	);
	
	
	
	
	
	
	
	jQuery(window).scroll( 
	
		function(){
			
			if(jQuery(window).scrollTop() > 3200 ){
				
				jQuery('.fclty-right>.std-pic').css({'transform':'scale(1)','opacity':'1'});
				
				
			}
			else{
				
				jQuery('.fclty-right>.std-pic').css({'transform':'scale(.1)','opacity':'.1'});
				
				
			}
			
			if(jQuery(window).scrollTop() > 3700 ){
				
				jQuery('.fclity-dygrm').css({'transform':'scale(1)','opacity':'1'});
				jQuery('.fclity-ststc').css({'transform':'scaleX(1)','opacity':'1'});
				
				
			}
			else{
				
				jQuery('.fclity-dygrm').css({'transform':'scale(.1)','opacity':'.1'});
				jQuery('.fclity-ststc').css({'transform':'scaleX(.1)','opacity':'.1'});
				
				
			}			
			
		}
	
	);
 






 
	
	
});
